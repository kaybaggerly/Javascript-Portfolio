(async function() {
    let jsonData = await getAPIData('https://overwatch-api.net/api/v1/hero/');
    let characterData = [];
    for (const stats in jsonData['data']) {
...
class OWCharacter {
  ...
  newButton.addEventListener("click", function() {
      let charId = prompt("Please enter a character ID");
      console.log(typeof charId);
      if (charId > 0 && charId <= 20) { // Or however many there are. 
        getAPIData`'https://overwatch-api.net/api/v1/hero/${charId}`)
      .then(result => {
      populateDOM(result);
      });
    } else {
        alert('There are no characters with that number. Try another one.');
    }
  }) 
